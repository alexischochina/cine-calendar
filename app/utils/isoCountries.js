// Mapping ISO 3166-1 alpha-2 → numérique (chaîne à 3 chiffres, zéros de tête inclus).
//
// Les entrées `calendar.countries` stockent des codes alpha-2 (source TMDB
// `production_countries[].iso_3166_1`, ex. « US », « GB »). Le fichier world-atlas
// (`countries-110m.json`) identifie chaque pays par son code ISO **numérique** en `feature.id`
// (ex. « 840 » pour les États-Unis). Cette table statique fait le pont entre les deux, sans
// dépendance ni appel réseau (offline-safe). Table officielle ISO 3166-1 (~249 entrées).
...
// Index inverse numérique → alpha-2 (construit une fois).
const NUMERIC_TO_ISO2 = Object.fromEntries(
    Object.entries(ISO2_TO_NUMERIC).map(([iso2, num]) => [num, iso2])
);

// Alpha-2 (« US ») → id numérique atlas (« 840 »). Code inconnu → null (le film est
// ignoré proprement sur la carte, mais reste compté ailleurs).
export const iso2ToNumeric = (code) =>
    ISO2_TO_NUMERIC[String(code || '').toUpperCase()] || null;

// Id numérique atlas (« 840 ») → alpha-2 (« US »). Utile pour relier une `feature` au pays.
export const numericToIso2 = (num) =>
    NUMERIC_TO_ISO2[String(num || '')] || null;
